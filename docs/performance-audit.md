...
| # | 优化项 | 优化空间 | 引入Bug风险 | 改变交互风险 | 推荐优先级 | 实施状态 |
|---|--------|----------|-------------|-------------|-----------|---------|
| 1 | 全站注入+启动即重型初始化 | ⚠️ 有限 | 🔴 高 | 🔴 高 | ❌ 不推荐 | ❌ 未实施 |
| 2 | 搜索链路每次输入都做双算法全量计算 | ✅ 有 | 🟡 中 | 🟢 低 | ✅ 推荐 | ✅ 已完成 |
| 3 | 虚拟滚动每次渲染都清空并重建可见DOM | ✅ 有 | 🟡 中 | 🟢 低 | ⚠️ 需谨慎 | ⏸️ 暂不实施 |
| 4 | 多处强制回流+多定时器动画 | ✅ 有 | 🟢 低 | 🟡 中 | ✅ 推荐 | ✅ 已完成 |
| 5 | 标签栏更新成本偏高且触发频繁 | ✅ 有 | 🟢 低 | 🟢 低 | ✅ 推荐 | ✅ 已完成 |
| 6 | 进入文件夹时重复全量扫描 | ✅ 有 | 🟢 低 | 🟢 低 | ✅ 推荐 | ✅ 已完成 |
| 7 | 后台广播和日志噪音 | ✅ 有 | 🟢 低 | 🟢 低 | ✅ 推荐 | ✅ 已完成 |
| 8 | 潜在监听器累积 | ⚠️ 有限 | 🟢 低 | 🟢 低 | ✅ 推荐 | ✅ 已完成 |
...
| 项目 | 预估工作量 | 核心改动 | 实施状态 |
|------|-----------|---------|---------|
| #7 后台广播优化 | 30min | `background.js` 中 `broadcastBookmarksChanged` 优先发 `loadedTabs` 并保留 MV3 兜底 | ✅ 已完成 |
| #7 日志开关 | 30min | 添加 `DEBUG` 常量，替换所有 `console.log` | ✅ 已完成 |
| #8 监听器泄漏 | 20min | `modal-manager-core.js` 中保存引用 + cleanup 移除 | ✅ 已完成 |
| #5 标签栏更新优化 | 1h | `ui-manager.js` 中加标签容器宽度和缓存签名 + 宽度缓存 | ✅ 已完成 |

### 第二批（中风险、中回报）

| 项目 | 预估工作量 | 核心改动 | 实施状态 |
|------|-----------|---------|---------|
| #2 条件 fallback | 1h | `search-engine.js` 中加阈值判断 | ✅ 已完成 |
| #6 parentId 索引 | 1.5h | `modal-manager-data.js` 中构建 `buildFolderIdMap` 并新增 `refreshFoldersForNavigation` | ✅ 已完成 |
| #4 合并回流 | 2h | `modal-manager-search.js` + `ui-manager.js` 中重构高度动画逻辑 | ✅ 已完成 |

### 第三批（高风险、低回报）

| 项目 | 预估工作量 | 核心改动 | 实施状态 |
|------|-----------|---------|---------|
| #3 虚拟滚动节点池 | 4h+ | `virtual-scroller.js` 大幅重构 | ⏸️ 暂不实施 |
| #1 按需初始化 | 8h+ | manifest + content-script + background 全面改造 | ❌ 不推荐实施 |
